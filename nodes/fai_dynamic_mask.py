...
class FAIDynamicMask:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "weight_type": (["linear", "ease_in", "ease_out", "ease_in_out"],)
            },
        }
    ...
    def generate_mask(self, width, height, frame_count, shape_transition, feather, weight_type):
        mask_sequence = []
        invert_mask_sequence = []

        for frame in range(frame_count):
            mask = self.create_mask(frame, frame_count, shape_transition, width, height, feather, weight_type)
            invert_mask = 1 - mask
    ...
    def create_mask(self, frame, frame_count, shape_transition, width, height, feather, weight_type):
        mask = np.zeros((height, width), dtype=np.float32)
        t = frame / (frame_count - 1)

        if weight_type == "linear":
            progress = t
        elif weight_type == "ease_in":
            progress = t ** 2.0
        elif weight_type == "ease_out":
            progress = t ** (1 / 2.0)
        elif weight_type == "ease_in_out":
